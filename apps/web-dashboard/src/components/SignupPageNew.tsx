...
const SignupPage: React.FC<SignupPageProps> = ({ onBack, onSuccess, navigateTo }) => {
  ...
  const validateForm = () => {
    ...
    if (userType === 'traffic-officer') {
      // Both fields are optional for traffic officers
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await authService.signup({
        ...formData,
        userType: userType as 'doctor' | 'staff' | 'patient' | 'traffic-officer'
      });
      ...
      <div className="w-full bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-4xl transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-300">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid gap-3 grid-cols-3 mb-6">
              {[
                ...
                { key: 'traffic-officer', widthKey: ['traffic-officer'], label: 'Traffic Officer', icon: <Shield className="h-4 w-4" /> }
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setUserType(newType);
                      setFormData(prev => ({ ...prev, country: '', medicalId: '', doctorId: '', staffType: '', vehicleNumber: '', junctionId: '', badgeNumber: '' }));
                    }}
          ...
          {userType === 'traffic-officer' && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-lifelink-text mb-2">Junction Location (Optional)</label>
                <input
                  type="text"
                  value={formData.junctionId}
                  onChange={(e) => handleInputChange('junctionId', e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-lifelink-primary focus:border-transparent"
                  placeholder="Main St & 4th Ave"
                />
                <p className="text-xs text-gray-500 mt-1">Enter your assigned junction/intersection location</p>
                {errors.junctionId && <p className="text-red-500 text-sm mt-1">{errors.junctionId}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-lifelink-text mb-2">Badge Number (Optional)</label>
                <input
                  type="text"
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Enter your traffic officer badge number</p>
                {errors.badgeNumber && <p className="text-red-500 text-sm mt-1">{errors.badgeNumber}</p>}
              </div>
            </div>
          )}
