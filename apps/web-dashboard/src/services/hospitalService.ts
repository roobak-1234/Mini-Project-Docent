...
class HospitalService {
  ...
  async generateCapabilitySummary(hospitalData: HospitalRegistrationData): Promise<string> {
    try {
      const prompt = `Generate a concise hospital capability summary for paramedics based on:
      - Hospital: ${hospitalData.name}
      - Type: ${hospitalData.type}
      - ICU Beds: ${hospitalData.icuBeds}
      - HDU Beds: ${hospitalData.hduBeds}
      - Isolation Beds: ${hospitalData.isolationBeds}
      - NICU Beds: ${hospitalData.nicuBeds}
      - PICU Beds: ${hospitalData.picuBeds}
      - Ventilators: ${hospitalData.ventilators}
      - OT Status: ${hospitalData.otStatus}
      - Specializations: ${Object.entries(hospitalData.specializations)
        .filter(([_, value]) => value)
        .map(([key]) => key)
        .join(', ')}
      
      Format: 2-3 sentences for ambulance crews.`;
